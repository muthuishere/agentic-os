...
`agentic-os window move Chrome --zone=1B` and a `window_move` call with
...
```sh
agentic-os serve mcp --groups=window,capture,exec   # a subset
agentic-os serve mcp --gui=off                      # only the screenless tools
```
